...
| **エンベディング** | OpenAI text-embedding-3-small | **Gemini Embedding（gemini-embedding-001）** |
...
hook_pool = strategy['hookVectors']
n1_demand_vec = np.array([v['vector'] for v in all_vectors if v['category'] == 'layer_demand' and v['layer'] == primary_layer][0])

# N本分を選択: 互いの sim <= 0.70（多様性）& N1需要 sim >= 0.45（刺さり度）
selected = []
pool = sorted(hook_pool, key=lambda h: -cosine_sim(np.array(h['vector']), n1_demand_vec))
...
# formatStrategy.articleLP の基準値
checkpoints = strategy['formatStrategy']['articleLP'].get('vectorCheckpoints', {
    'fv_vs_concept': 0.50,
    'fv_vs_competitor': 0.55,
    'inter_lp_diversity': 0.70
})

# 各記事LPのFVコピー（3パターン分）をベクトル化
lp_vecs = []
for spec in lpSpecs:
    for pattern_key in ['patternA', 'patternB', 'patternC']:
        fv = spec['fvPatterns'][pattern_key]
        text = fv['mainCopy'] + ' ' + fv['subCopy']
        vec = embed(text)
        lp_vecs.append({
            'lpIndex': spec['lpIndex'],
            'pattern': pattern_key,
            'vector': vec,
            'text': fv['mainCopy']
        })
...
print("\n=== 検証1: コンセプト一貫性 ===")
for lv in lp_vecs:
    sim = cosine_sim(lv['vector'], concept_vec)
    ok = sim >= 0.50
    if not ok: all_pass = False
    print(f"  LP{lv['lpIndex']} {lv['pattern']} 「{lv['text'][:20]}」× コンセプト: sim={sim:.3f} {'✅' if ok else '⚠️'}")
    gate_results.append({'lpIndex': lv['lpIndex'], 'pattern': lv['pattern'], 'check': 'concept', 'sim': sim, 'pass': ok})

# === 検証2: 競合差別化（FVコピー × competitor_avg <= 0.55）===
print("\n=== 検証2: 競合差別化 ===")
for lv in lp_vecs:
    avg = np.mean([cosine_sim(lv['vector'], c) for c in comp_vecs]) if comp_vecs else 0
    ok = avg <= 0.55
    if not ok: all_pass = False
    print(f"  LP{lv['lpIndex']} {lv['pattern']} × 競合平均: sim={avg:.3f} {'✅' if ok else '⚠️'}")
    gate_results.append({'lpIndex': lv['lpIndex'], 'pattern': lv['pattern'], 'check': 'competitor', 'sim': avg, 'pass': ok})

# === 検証3: 記事LP間多様性（inter_lp sim <= 0.70）===
print("\n=== 検証3: 記事LP間多様性 ===")
# 同一パターン間で比較（LP1のA vs LP2のA, LP1のB vs LP2のB, ...）
for pattern in ['patternA', 'patternB', 'patternC']:
    vecs_for_pattern = [lv for lv in lp_vecs if lv['pattern'] == pattern]
    for i in range(len(vecs_for_pattern)):
        for j in range(i+1, len(vecs_for_pattern)):
            sim = cosine_sim(vecs_for_pattern[i]['vector'], vecs_for_pattern[j]['vector'])
            if sim > 0.70:
                all_pass = False
                print(f"  ⚠️ LP{vecs_for_pattern[i]['lpIndex']} vs LP{vecs_for_pattern[j]['lpIndex']} ({pattern}): sim={sim:.3f}")

# === 検証4: N1需要 刺さり度（参考値）===
print("\n=== 検証4: N1需要 刺さり度（参考）===")
for lv in lp_vecs:
    sim = cosine_sim(lv['vector'], n1_demand_vec)
    print(f"  LP{lv['lpIndex']} {lv['pattern']}: sim={sim:.3f}")
...
- **コンセプト一貫性 FAIL** → FVコピーをコンセプトに寄せて再生成
- **競合差別化 FAIL** → フック角度変更、または表現差別化
- **多様性 FAIL** → 類似ペアの片方のフック角度/仮想敵を変更
- 自動修正後に再検証。3回トライしてもFAILなら、パクに報告
...
| 6 | **KV画像** | 世界観 | プロダクトUI or 成果暗示（Gemini生成 or 実素材）|
...
- **CTA1つのみ**: 迷わせない。「無料で試す」系の低ハードル
- **ロード速度**: 画像は1枚600KB以下。遅延読み込み不要（FVは即表示）
...
### 法則5: FV素材活用ルール（バナーから学ぶ）

- **商材のAIキャラ/マスコットがあれば必ずFVに配置**: 右下に小さく（64px）、親しみやすさと認知を両立
- **信頼バッジはPNG画像で生成**: HTMLテキストではなく、PILで透過PNGとして生成し`<img>`で配置。Gemini生成は白背景が残るためPIL推奨
- **バッジサイズは大きめ（80px）**: FV内で存在感を出す。金×紺のプレミアム感
- **数字の強弱を明確に**: 重要な数字（実績数値）は `big-num` クラスで36px、通常コピーは22px。メリハリが命
- **FV画像はテキストなしで生成**: 「NO TEXT NO WORDS NO LETTERS」をプロンプトに必ず含め、テキストはHTMLオーバーレイで重ねる（AI生成テキスト×HTMLテキストの二重表示を防ぐ）
- **overlay は中央寄せ + radial gradient**: `justify-content: center` で情報を画面中央に配置。背景は radial-gradient で中央が濃く、端が薄い
